...
public:
    ...
    explicit CatchTreeItem(const QString &name = QString(), const QString &filePath = QString(),
...
private:
    bool modifyTestSetContent(const CatchParseResult *result);
    QList<TestConfiguration *> getTestConfigurations(bool ignoreCheckState) const;
    CatchTreeItem::TestStates m_state;
};

class CatchCodeLocationAndType : public TestCodeLocationAndType
{
public:
    CatchTreeItem::TestStates m_state;
};

typedef QVector<CatchCodeLocationAndType> CatchCodeLocationList;

struct CatchCaseSpec
{
    QString testCaseName;
    bool parameterized;
    bool typed;
    bool disabled;
    // TODO for catch maybe hidden etc.
