...
void CatchOutputReader::processOutput(const QByteArray &outputLine)
{
    ...
    if (!line.startsWith('[')) {
        ...
        if (iterations.exactMatch(line)) {
            ...
            TestResultPtr testResult = TestResultPtr(new CatchResult(m_projectFile));
...
{
    CatchResult *result = new CatchResult(m_executable, m_projectFile, m_currentTestName);
...
void CatchOutputReader::setCurrentTestSet(const QString &testSet)
{
    m_currentTestSet = testSet;
}

void CatchOutputReader::setCurrentTestName(const QString &testName)
{
