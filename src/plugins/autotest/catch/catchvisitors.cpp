...
CatchVisitor::CatchVisitor(CPlusPlus::Document::Ptr doc)
    : CPlusPlus::ASTVisitor(doc->translationUnit())
    , m_document(doc)
{
}

bool CatchVisitor::visit(CPlusPlus::FunctionDefinitionAST *ast)
{
    ...
    if (!CatchUtils::isCatchMacro(prettyName))
        return false;
    ...
    if (testCaseNameArg && testNameArg) {
        ...
        CatchCodeLocationAndType locationAndType;
        locationAndType.m_name = testName;
        locationAndType.m_line = line;
        locationAndType.m_column = column - 1;
        locationAndType.m_type = TestTreeItem::TestFunctionOrSet;
        locationAndType.m_state = disabled ? CatchTreeItem::Disabled
                                           : CatchTreeItem::Enabled;
        CatchCaseSpec spec;
        spec.testCaseName = testCaseName;
        spec.parameterized = false;
        spec.typed = false;
        spec.disabled = disabledCase;
        m_catchFunctions[spec].append(locationAndType);
    }

    return false;
}

QString CatchVisitor::enclosingNamespaces(CPlusPlus::Symbol *symbol) const
{
    QString enclosing;
    if (!symbol)
        return enclosing;
